$(document).ready(function () {
  ...
  //funciones
  //funcion de rectangulo
  $(document).on("click", ".rectangulo", function () {
    const drawRect = (rect) => {
      ctx.beginPath();
      ...
      // Calcula el área del rectángulo en centímetros cuadrados
      const baseCm = rect.width * pixelToCm;
      const alturaCm = rect.height * pixelToCm;
      const areaCm2 = baseCm * alturaCm;
      const areaText = `Área: ${areaCm2.toFixed(2)} cm²`;
      
      // Obtén el número de pisos desde un input en tu HTML
    ...
    const drawAllLegends = () => {
      ...
      rectangles.forEach((rect, index) => {
        ...
        const areaCm2 = (
          (rect.width * pixelToCm * rect.height * pixelToCm) /
          2
        ).toFixed(2); // Calcular el área como base por altura sobre 2 y convertir a cm²
        const npisos = document.getElementById("npisos").value;
        const Ac =
          (parseFloat(areaCm2) * parseFloat(npisos) * 1000) / (0.45 * 210);
        ctx.fillText(`${rect.dimensions}`, `Área: ${areaCm2} m²`, x, textY);
        ctx.fillText(`${rect.dimensions}`, `AC: ${Ac} m²`, x, textY);
      });
    };
    ...
    canvas.addEventListener("mouseup", (e) => {
      if (selectedTool === "rectangle") {
        ...
        const newRect = {
          ...
          brushWidth: brushWidth,
          fill: fillColor.checked,
          dimensions: `Ancho: ${widthCm.toFixed(2)} m, Alto: ${heightCm.toFixed(
            2
          )} m`,
        };
...
/*       legend.style.display = "none"; // Ocultar leyenda cuando se cambia de herramienta */
    ...
    document.querySelector("#borrador").addEventListener("click", () => {
      ...
      document.querySelector(".tool.active").classList.remove("active");
      document.querySelector("#borrador").classList.add("active");
/*       legend.style.display = "none"; // Ocultar leyenda cuando se cambia de herramienta */
    });
